...
RELEASE_LOCK_MODULE_VERSION = "2.2.0"
STABLE_RUNTIME_VERSION = "2.2.0"
...
def create_stable_capability_inventory() -> dict:
    return {
        "capability_inventory_version": "2.2.0",
...
def create_stable_artifact_contract() -> dict:
    return {
        "artifact_contract_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
        "artifact_groups": {
            "core_runtime": [
                "run_log.json",
...
def create_stable_adapter_boundary_contract() -> dict:
    return {
        "adapter_boundary_contract_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
        "adapters": {
            "noop": {
...
def create_stable_safety_doctrine_lock() -> dict:
    return {
        "safety_doctrine_lock_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
...
def create_stable_approval_flow_lock() -> dict:
    return {
        "approval_flow_lock_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
        "approval_flow_steps": [
            "1. command intake",
...
def create_known_limitations_record() -> dict:
    return {
        "known_limitations_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
        "limitations": [
...
def create_next_phase_handoff_record() -> dict:
    return {
        "next_phase_handoff_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
        "current_phase": "Live Execution Telemetry and Abort Controls",
        "next_phase": "Post-Run Audit Proof Expansion",
        "recommended_next_builds": [
            "v2.3 post-run audit proof expansion",
...
def create_release_readiness_summary() -> dict:
    return {
        "release_readiness_summary_version": "2.2.0",
        "runtime_version": STABLE_RUNTIME_VERSION,
...
def create_stable_release_manifest() -> dict:
    manifest = {
        "stable_release_manifest_version": "2.2.0",
        "runtime_name": STABLE_RUNTIME_NAME,
        "runtime_version": STABLE_RUNTIME_VERSION,
...
def verify_stable_release_manifest(release_manifest: dict) -> dict:
    ...
    return {
        "stable_release_verification_version": "2.2.0",
...
def create_release_lock_bundle() -> dict:
    ...
    return {
        "release_lock_bundle_version": "2.2.0",
...
def write_release_lock(result: dict, output_dir: str | Path, run_label: str = "station-chief-runtime") -> dict:
    ...
    manifest = {
        "release_lock_manifest_version": "2.2.0",
        "run_id": run_id,
        ...
        "execution_authorized": False,
        "release_status": "STABLE_LOCKED",
        "note": "Station Chief Runtime v2.2.0 stable release lock does not execute repo patches by itself."
    }
