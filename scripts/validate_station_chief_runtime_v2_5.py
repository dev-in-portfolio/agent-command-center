...
def validate_source_strings(errors: list[str]) -> None:
    ...
    external_preview = read_text("10_runtime/station_chief_controlled_external_tool_adapter_preview.py")
    ...
    for needle in [
        ...
        "attach_controlled_external_tool_adapter_preview",
        "write_controlled_external_tool_adapter_preview",
        ...
        "--external-tool-preview-schema",
        "--controlled-external-tool-preview",
        "--write-controlled-external-tool-preview",
        "--external-tool-label",
        "--external-tool-id",
        "--external-tool-confirm-token",
        "--external-tool-requested-tool",
        "--external-tool-requested-action",
        "--external-tool-request-label",
        "--external-tool-request-payload-json",
        "--external-tool-response-preview-json",
        "--external-tool-abort-reason",
        ...
        "controlled_external_tool_adapter_preview_available",
        "controlled_external_tool_adapter_preview_only",
        "controlled_external_tool_adapter_preview_requires_token",
        "controlled_external_tool_adapter_preview_does_not_invoke_external_tools",
        "controlled_external_tool_adapter_preview_does_not_call_live_apis",
        "controlled_external_tool_adapter_preview_does_not_use_network_access",
        "controlled_external_tool_adapter_preview_does_not_open_sockets",
        "controlled_external_tool_adapter_preview_does_not_modify_repo_files",
        "permissioned_external_api_dry_run_preview_not_yet_active",
        "controlled_external_tool_adapter_preview_manifest",
        "permissioned_external_api_dry_run_preview_readiness_bridge",
        "controlled_external_tool_adapter_preview_bundle",
        "controlled_external_tool_adapter_preview_schema",
        ...
        'Next step: build permissioned external API dry-run preview.',
    ]:
    ...
    for needle in [
        ...
        'supports_controlled_external_tool_adapter_preview',
        'controlled_external_tool_adapter_preview_requires_specific_token',
        ...
        'controlled_external_tool_adapter_preview_requires_separate_gate',
        'YES_I_APPROVE_POST_RUN_AUDIT_PROOF_EXPANSION',
        'YES_I_APPROVE_MULTI_WORKER_SANDBOX_COORDINATION',
        'YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW',
    ]:
    ...
    for needle in [
        'MULTI_WORKER_SANDBOX_COORDINATION_MODULE_VERSION = "2.5.0"',
    ...
    for needle in [
        ...
        'CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_STATUS',
        'CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_PHASE',
        'CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_APPROVAL_TOKEN',
        'canonical_json',
        'sha256_digest',
        'normalize_external_tool_label',
        'generate_external_tool_preview_id',
        'create_controlled_external_tool_adapter_preview_schema',
        'create_external_tool_adapter_preview_approval_gate',
        'create_external_tool_dry_run_adapter_registry',
        'create_per_tool_external_permission_gate',
        'create_external_request_preview_contract',
        'create_external_response_validation_schema',
        'create_external_response_validation_preview_result',
        'create_external_tool_abort_contract',
        'create_external_tool_audit_proof',
        'create_external_tool_preview_ledger',
        'create_external_tool_preview_readiness_summary',
        'create_permissioned_external_api_dry_run_preview_readiness_bridge',
        'create_controlled_external_tool_adapter_preview_bundle',
    ]:
        require(errors, needle in external_preview, f"controlled external preview module missing string: {needle}")
    ...
    for needle in [
        ...
        require(errors, needle not in external_preview, f"forbidden string '{needle}' found in controlled external preview module")

    forbidden_common = ["requests", "urllib.request", "os.system", "pip install", "npm install", "API key", "import subprocess"]
    for rel in [
        "10_runtime/station_chief_runtime.py",
        "10_runtime/station_chief_adapters.py",
        "10_runtime/station_chief_execution_profiles.py",
...
def validate_demo_and_fixture(errors: list[str]) -> None:
    ...
    if demo:
        require(errors, demo.get("station_chief_runtime_version") == "2.5.0", "demo runtime version mismatch")
        require(errors, demo.get("runtime_status") == "controlled_external_tool_adapter_preview", "demo runtime_status mismatch")
    ...
    if fixture:
        ...
        require(errors, fixture.get("runtime_version") == "2.5.0", "fixture runtime version mismatch")
    ...
    if fixture_runner:
        ...
        require(errors, fixture_runner.get("runtime_version") == "2.5.0", "fixture runner version mismatch")
...
def validate_overlay_and_adapter_views(errors: list[str]) -> None:
    ...
    if adapters is not None:
        require(errors, adapters.get("adapter_module_version") == "2.5.0", "adapter module version mismatch")
        ...
        require(errors, adapters.get("supports_controlled_external_tool_adapter_preview") is True, "adapter controlled external preview support mismatch")
        require(errors, adapters.get("controlled_external_tool_adapter_preview_requires_specific_token") is True, "adapter controlled external preview token mismatch")
        require(errors, adapters.get("external_tool_invocation_allowed") is False, "adapter external tool invocation flag mismatch")
        require(errors, adapters.get("live_api_call_allowed") is False, "adapter live api flag mismatch")
        require(errors, adapters.get("network_access_allowed") is False, "adapter network flag mismatch")
        require(errors, adapters.get("socket_access_allowed") is False, "adapter socket flag mismatch")
        ...
        require(errors, noop.get("supports_controlled_external_tool_adapter_preview") is True, "noop controlled external preview support mismatch")
        ...
        require(errors, scoped.get("supports_controlled_external_tool_adapter_preview") is False, "scoped patch controlled external preview support mismatch")
        require(errors, scoped.get("controlled_external_tool_adapter_preview_requires_separate_gate") is True, "scoped patch controlled external preview gate mismatch")
...
def validate_schema_views(errors: list[str]) -> None:
    ...
    if post_run_schema:
        require(errors, post_run_schema.get("post_run_audit_expansion_schema_version") == "2.5.0", "post-run schema version mismatch")
    ...
    if multi_schema:
        require(errors, multi_schema.get("multi_worker_sandbox_coordination_schema_version") == "2.5.0", "multi-worker schema version mismatch")
    ...
    code, out, err = run_command(["python3", "10_runtime/station_chief_runtime.py", "--external-tool-preview-schema"])
    require(errors, code == 0, f"--external-tool-preview-schema failed: {err}")
    external_schema = parse_json(out, "--external-tool-preview-schema", errors)
    if external_schema:
        require(errors, external_schema.get("controlled_external_tool_adapter_preview_schema_version") == "2.5.0", "external preview schema version mismatch")
        require(errors, external_schema.get("schema_status") == "CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_ONLY", "external preview schema status mismatch")
        for key in [
            "external_tool_adapter_preview_approval_gate",
            "external_tool_dry_run_adapter_registry",
            "per_tool_external_permission_gate",
            ...
            require(errors, key in external_schema.get("required_sections", []), f"external preview required_sections missing {key}")
        require(errors, "local_external_tool_preview" in external_schema.get("allowed_preview_modes", []), "external preview allowed_preview_modes missing local_external_tool_preview")
        require(errors, "approved_external_tool_adapter_preview_records" in external_schema.get("allowed_preview_modes", []), "external preview allowed_preview_modes missing approved_external_tool_adapter_preview_records")
        for key in [
            ...
            require(errors, key in external_schema.get("blocked_preview_modes", []), f"external preview blocked_preview_modes missing {key}")
        require(errors, "YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW" in external_schema.get("required_confirmation_tokens", []), "external preview token missing")
        require(errors, external_schema.get("baseline_preserved") is True, "external preview schema baseline mismatch")
        require(errors, external_schema.get("external_actions_taken") is False, "external preview schema external_actions mismatch")
        require(errors, external_schema.get("external_tool_invoked") is False, "external preview schema tool invocation mismatch")
        require(errors, external_schema.get("live_api_call_performed") is False, "external preview schema live api mismatch")
        require(errors, external_schema.get("network_access_performed") is False, "external preview schema network mismatch")
        require(errors, external_schema.get("socket_opened") is False, "external preview schema socket mismatch")
        require(errors, external_schema.get("repo_files_modified") is False, "external preview schema repo mismatch")
        require(errors, external_schema.get("deployment_performed") is False, "external preview schema deployment mismatch")
        require(errors, external_schema.get("execution_authorized") is False, "external preview schema execution mismatch")
...
def validate_post_run_flow(errors: list[str]) -> None:
    ...
    if result:
        ...
        require(errors, result["post_run_audit_expansion_bundle"]["post_run_audit_expansion_bundle_version"] == "2.5.0", "post-run bundle version mismatch")
...
def validate_multi_worker_flow(errors: list[str]) -> None:
    ...
    if result:
        ...
        require(errors, result["multi_worker_sandbox_coordination_bundle"]["multi_worker_sandbox_coordination_bundle_version"] == "2.5.0", "multi-worker bundle version mismatch")
...
def validate_controlled_external_tool_preview_flow(errors: list[str]) -> None:
    code, out, err = run_command([
        "python3",
        "10_runtime/station_chief_runtime.py",
        "--command",
    ...
    require(errors, code == 0, f"controlled external preview default failed: {err}")
    result = parse_json(out, "controlled external preview default", errors)
    if result:
        for key in [
            "controlled_external_tool_adapter_preview_bundle",
            "controlled_external_tool_adapter_preview_schema",
            ...
            require(errors, key in result, f"controlled external preview default missing {key}")
        require(errors, result["controlled_external_tool_adapter_preview_bundle"]["controlled_external_tool_adapter_preview_bundle_version"] == "2.5.0", "controlled external preview bundle version mismatch")
        require(errors, result["external_tool_adapter_preview_approval_gate"]["gate_status"] == "BLOCKED_PENDING_EXTERNAL_TOOL_ADAPTER_PREVIEW_APPROVAL", "controlled external preview default gate mismatch")
        require(errors, result["external_tool_adapter_preview_approval_gate"]["confirmation_token_valid"] is False, "controlled external preview default token mismatch")
        require(errors, result["external_tool_dry_run_adapter_registry"]["registry_status"] == "BLOCKED", "controlled external preview default registry mismatch")
        require(errors, result["per_tool_external_permission_gate"]["permission_status"] == "BLOCKED", "controlled external preview default permission mismatch")
        require(errors, result["external_request_preview_contract"]["request_contract_status"] == "BLOCKED", "controlled external preview default request mismatch")
        require(errors, result["external_tool_audit_proof"]["audit_status"] == "BLOCKED", "controlled external preview default audit mismatch")
        require(errors, result["external_tool_preview_readiness_summary"]["readiness_status"] == "BLOCKED", "controlled external preview default readiness mismatch")
        require(errors, result.get("external_actions_taken") is False, "controlled external preview default actions mismatch")
        require(errors, result.get("external_tool_invoked") is False, "controlled external preview default tool mismatch")
        require(errors, result.get("live_api_call_performed") is False, "controlled external preview default live api mismatch")
        require(errors, result.get("network_access_performed") is False, "controlled external preview default network mismatch")
        require(errors, result.get("socket_opened") is False, "controlled external preview default socket mismatch")
        require(errors, result.get("repo_files_modified") is False, "controlled external preview default repo mismatch")
        require(errors, result.get("execution_authorized") is False, "controlled external preview default execution mismatch")
    ...
    require(errors, code == 0, f"controlled external preview approved failed: {err}")
    result = parse_json(out, "controlled external preview approved", errors)
    if result:
        require(errors, result["external_tool_adapter_preview_approval_gate"]["gate_status"] == "APPROVED_FOR_EXTERNAL_TOOL_ADAPTER_PREVIEW_RECORDS", "controlled external preview approved gate mismatch")
        require(errors, result["external_tool_adapter_preview_approval_gate"]["confirmation_token_valid"] is True, "controlled external preview approved token mismatch")
        require(errors, result["external_tool_dry_run_adapter_registry"]["registry_status"] == "REGISTRY_CREATED", "controlled external preview approved registry mismatch")
        require(errors, result["per_tool_external_permission_gate"]["permission_status"] == "PREVIEW_PERMISSION_BOUND", "controlled external preview approved permission mismatch")
        require(errors, result["external_request_preview_contract"]["request_contract_status"] == "CONTRACT_CREATED", "controlled external preview approved request mismatch")
        require(errors, result["external_response_validation_preview_result"]["validation_status"] == "PASS", "controlled external preview approved validation mismatch")
        require(errors, result["external_tool_abort_contract"]["contract_status"] == "READY", "controlled external preview approved abort mismatch")
        require(errors, result["external_tool_audit_proof"]["audit_status"] == "PASS", "controlled external preview approved audit mismatch")
        require(errors, result["external_tool_preview_ledger"]["ledger_status"] == "CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_LEDGER", "controlled external preview approved ledger mismatch")
        require(errors, result["external_tool_preview_readiness_summary"]["readiness_status"] == "READY_FOR_NEXT_LAYER", "controlled external preview approved readiness mismatch")
        require(errors, result["permissioned_external_api_dry_run_preview_readiness_bridge"]["next_layer"] == "Permissioned External API Dry-Run Preview", "controlled external preview bridge next layer mismatch")
        require(errors, result["permissioned_external_api_dry_run_preview_readiness_bridge"]["ready_for_permissioned_external_api_dry_run_preview"] is True, "controlled external preview bridge readiness mismatch")
        require(errors, result.get("external_actions_taken") is False, "controlled external preview approved actions mismatch")
        require(errors, result.get("external_tool_invoked") is False, "controlled external preview approved tool mismatch")
        require(errors, result.get("live_api_call_performed") is False, "controlled external preview approved live api mismatch")
        require(errors, result.get("network_access_performed") is False, "controlled external preview approved network mismatch")
        require(errors, result.get("socket_opened") is False, "controlled external preview approved socket mismatch")
        require(errors, result.get("repo_files_modified") is False, "controlled external preview approved repo mismatch")
        require(errors, result.get("execution_authorized") is False, "controlled external preview approved execution mismatch")
    ...
    code, out, err = run_command([
        ...
        "YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW",
        "--external-tool-response-preview-json",
        '{"tool_id":"web_search_preview","response_status":"PASS","response_payload":{"bad":"api_key=123"},"response_digest":"bad","external_actions_taken":false,"live_api_call_performed":false,"repo_files_modified":false,"execution_authorized":false}',
    ])
    require(errors, code == 0, f"controlled external preview unsafe response failed: {err}")
    result = parse_json(out, "controlled external preview unsafe response", errors)
    if result:
        require(errors, result["external_response_validation_preview_result"]["validation_status"] == "BLOCKED", "controlled external preview unsafe validation mismatch")
        require(errors, result["external_tool_audit_proof"]["audit_status"] == "BLOCKED", "controlled external preview unsafe audit mismatch")
        require(errors, result["external_tool_preview_readiness_summary"]["readiness_status"] == "BLOCKED", "controlled external preview unsafe readiness mismatch")
        require(errors, result.get("external_actions_taken") is False, "controlled external preview unsafe actions mismatch")
        require(errors, result.get("external_tool_invoked") is False, "controlled external preview unsafe tool mismatch")
        require(errors, result.get("live_api_call_performed") is False, "controlled external preview unsafe live api mismatch")
        require(errors, result.get("network_access_performed") is False, "controlled external preview unsafe network mismatch")
        require(errors, result.get("socket_opened") is False, "controlled external preview unsafe socket mismatch")
        require(errors, result.get("repo_files_modified") is False, "controlled external preview unsafe repo mismatch")
        require(errors, result.get("execution_authorized") is False, "controlled external preview unsafe execution mismatch")

    code, out, err = run_command([
        "python3",
        "10_runtime/station_chief_runtime.py",
        "--command",
        "build permissioned external API dry-run preview",
        "--controlled-external-tool-preview",
        "--external-tool-confirm-token",
        "YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW",
    ])
    require(errors, code == 0, f"controlled external preview next-layer failed: {err}")
    result = parse_json(out, "controlled external preview next-layer", errors)
    if result:
        require(errors, result["external_tool_preview_readiness_summary"]["readiness_status"] == "READY_FOR_NEXT_LAYER", "controlled external preview next-layer readiness mismatch")
        require(errors, result["external_tool_preview_readiness_summary"]["next_layer"] == "Permissioned External API Dry-Run Preview", "controlled external preview next-layer summary mismatch")
        require(errors, result["permissioned_external_api_dry_run_preview_readiness_bridge"]["next_layer"] == "Permissioned External API Dry-Run Preview", "controlled external preview next-layer bridge mismatch")
        require(errors, result["permissioned_external_api_dry_run_preview_readiness_bridge"]["ready_for_permissioned_external_api_dry_run_preview"] is True, "controlled external preview next-layer bridge readiness mismatch")
        require(errors, result["external_tool_audit_proof"]["audit_status"] == "PASS", "controlled external preview next-layer audit mismatch")
        require(errors, result.get("external_actions_taken") is False, "controlled external preview next-layer actions mismatch")
        require(errors, result.get("external_tool_invoked") is False, "controlled external preview next-layer tool mismatch")
        require(errors, result.get("live_api_call_performed") is False, "controlled external preview next-layer live api mismatch")
        require(errors, result.get("network_access_performed") is False, "controlled external preview next-layer network mismatch")
        require(errors, result.get("socket_opened") is False, "controlled external preview next-layer socket mismatch")
        require(errors, result.get("repo_files_modified") is False, "controlled external preview next-layer repo mismatch")
...
def validate_write_flows(errors: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if result:
            ...
            require(errors, manifest.get("runtime_version") == "2.5.0", "post-run manifest runtime_version mismatch")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        require(errors, code == 0, f"write controlled external preview failed: {err}")
        result = parse_json(out, "write controlled external preview", errors)
        if result:
            require(errors, "controlled_external_tool_adapter_preview_write_summary" in result, "write controlled external preview summary missing")
            summary = result["controlled_external_tool_adapter_preview_write_summary"]
            written = summary.get("files_written", [])
            for name in [
                ...
                require(errors, name in written, f"write controlled external preview missing file {name}")
            manifest = json.loads((Path(summary["controlled_external_tool_adapter_preview_dir"]) / "controlled_external_tool_adapter_preview_manifest.json").read_text())
            require(errors, manifest.get("runtime_version") == "2.5.0", "controlled external preview manifest runtime_version mismatch")
            require(errors, manifest.get("status") == "CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW_ONLY", "controlled external preview manifest status mismatch")
            require(errors, manifest.get("external_actions_taken") is False, "controlled external preview manifest external_actions mismatch")
            require(errors, manifest.get("external_tool_invoked") is False, "controlled external preview manifest tool invocation mismatch")
            require(errors, manifest.get("live_api_call_performed") is False, "controlled external preview manifest live api mismatch")
            require(errors, manifest.get("network_access_performed") is False, "controlled external preview manifest network mismatch")
            require(errors, manifest.get("socket_opened") is False, "controlled external preview manifest socket mismatch")
            require(errors, manifest.get("repo_files_modified") is False, "controlled external preview manifest repo mismatch")
            require(errors, manifest.get("execution_authorized") is False, "controlled external preview manifest execution mismatch")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if result:
            ...
            require(errors, manifest.get("runtime_version") == "2.5.0", "multi-worker manifest runtime_version mismatch")
            ...
            require(errors, manifest.get("external_tool_invoked") is False, "multi-worker manifest tool invocation mismatch")
            require(errors, manifest.get("live_api_call_performed") is False, "multi-worker manifest live api mismatch")
            require(errors, manifest.get("network_access_performed") is False, "multi-worker manifest network mismatch")
            require(errors, manifest.get("socket_opened") is False, "multi-worker manifest socket mismatch")
    ...
    with tempfile.TemporaryDirectory() as run_dir, tempfile.TemporaryDirectory() as registry_dir:
        ...
        if result:
            ...
            require(errors, summary.get("run_id", "").startswith("station-chief-v2-5-check-please-"), "artifact run_id prefix mismatch")
            ...
            require(errors, manifest.get("artifact_type") == "station_chief_runtime_v2_5_artifacts", "artifact manifest type mismatch")
            require(errors, manifest.get("runtime_version") == "2.5.0", "artifact manifest runtime_version mismatch")
            for key in [
                "multi_worker_sandbox_coordination_schema",
                "multi_worker_coordination_approval_gate",
                "sandbox_worker_roster",
            ...
            require(errors, registry.get("registry_version") == "2.5.0", "registry version mismatch")
            require(errors, len(registry.get("runs", [])) >= 1, "registry runs mismatch")
            require(errors, index.get("index_version") == "2.5.0", "runtime index version mismatch")
    ...
    with tempfile.TemporaryDirectory() as run_dir, tempfile.TemporaryDirectory() as registry_dir:
        ...
        require(errors, code == 0, f"write artifacts with controlled external preview failed: {err}")
        result = parse_json(out, "write artifacts with controlled external preview", errors)
        if result:
            summary = result.get("artifact_write_summary", {})
            require(errors, summary.get("run_id", "").startswith("station-chief-v2-5-check-please-"), "controlled artifact run_id prefix mismatch")
            artifact_dir = Path(summary.get("artifact_dir", ""))
            require(errors, artifact_dir.exists(), "controlled artifact dir missing")
            require(errors, summary.get("registry_updated") is True, "controlled artifact registry updated mismatch")
            for name in [
                "controlled_external_tool_adapter_preview_bundle.json",
                "controlled_external_tool_adapter_preview_schema.json",
                "external_tool_adapter_preview_approval_gate.json",
                ...
                require(errors, (artifact_dir / name).exists(), f"controlled artifact missing {name}")
            manifest = json.loads((artifact_dir / "manifest.json").read_text())
            require(errors, manifest.get("artifact_type") == "station_chief_runtime_v2_5_artifacts", "controlled artifact manifest type mismatch")
            require(errors, manifest.get("runtime_version") == "2.5.0", "controlled artifact manifest runtime_version mismatch")
            for key in [
                "controlled_external_tool_adapter_preview_schema",
                "external_tool_adapter_preview_approval_gate",
                "external_tool_dry_run_adapter_registry",
                ...
                require(errors, manifest.get(key) is True, f"controlled artifact manifest flag mismatch: {key}")
            registry = json.loads((Path(registry_dir) / "run_registry.json").read_text())
            index = json.loads((Path(registry_dir) / "runtime_index.json").read_text())
            require(errors, registry.get("registry_version") == "2.5.0", "controlled registry version mismatch")
            require(errors, len(registry.get("runs", [])) >= 1, "controlled registry runs mismatch")
            require(errors, index.get("index_version") == "2.5.0", "controlled runtime index version mismatch")
            require(errors, index.get("run_count", 0) >= 1, "controlled runtime index run count mismatch")


def validate_regressions(errors: list[str]) -> None:
    commands = [
        (["python3", "10_runtime/station_chief_runtime.py", "--stable-release-manifest"], lambda data: require(errors, data.get("runtime_version") == "2.5.0", "stable manifest runtime version mismatch")),
        ...
        (["python3", "10_runtime/station_chief_runtime.py", "--command", "check please", "--json"], lambda data: require(errors, data.get("station_chief_runtime_version") == "2.5.0", "json runtime version mismatch")),
        ...
        (["python3", "10_runtime/station_chief_runtime.py", "--command", "check please", "--controlled-external-tool-preview"], lambda data: require(errors, "controlled_external_tool_adapter_preview_bundle" in data, "controlled external preview bundle missing")),
        (["python3", "10_runtime/station_chief_runtime.py", "--command", "check please", "--controlled-external-tool-preview", "--external-tool-confirm-token", "YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW"], lambda data: require(errors, data.get("external_tool_audit_proof", {}).get("audit_status") == "PASS", "controlled external preview audit mismatch")),
        (["python3", "10_runtime/station_chief_runtime.py", "--command", "build permissioned external API dry-run preview", "--controlled-external-tool-preview", "--external-tool-confirm-token", "YES_I_APPROVE_CONTROLLED_EXTERNAL_TOOL_ADAPTER_PREVIEW"], lambda data: require(errors, data.get("external_tool_preview_readiness_summary", {}).get("readiness_status") == "READY_FOR_NEXT_LAYER", "controlled external preview next layer mismatch")),
...
def validate_documents(errors: list[str]) -> None:
    ...
    report = read_text("09_exports/station_chief_runtime_v2_5_report.md")
    for text, label in [
        (readme, "README"),
    ...
    readme_needles = [
        "Station Chief Runtime upgraded to v2.5.0.",
        "Controlled external tool adapter preview added.",
    ...
    for needle in report_needles:
        require(errors, needle in report, f"v2.5 report missing string: {needle}")
...
def main() -> None:
    ...
    print("Manual scope check required: confirm git diff contains only the allowed Station Chief v2.5 runtime files.")
    if errors:
        for error in errors:
            print(error)
        print("FAIL")
    ...
    print("PASS: Station Chief Runtime v2.5 valid.")
